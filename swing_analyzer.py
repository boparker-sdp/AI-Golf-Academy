...
def analyze_diagnostic_swing(video_path, club_type=None):
        ...
        while cap.isOpened():
            ...
            if res.pose_landmarks:
                ...
                if is_downswing:
                    ...
                    if s_w < (h_w * 0.85): seq_status = "SHOULDER SPIN (PULL)"
                    else: seq_status = "HIP LEAD (PRO)"

                # Big Text Overlays
                col = (0, 255, 0) if "PRO" in seq_status else (0, 0, 255)
                cv2.putText(frame, f"SEQ: {seq_status}", (50, int(h*0.1)), 2, fs*1.2, (0,0,0), thick+2)
                cv2.putText(frame, f"SEQ: {seq_status}", (50, int(h*0.1)), 2, fs*1.2, col, thick)
                
                head_err = abs(lm[0].y - addr_head_y)
                h_col = (0, 255, 0) if head_err < 0.04 else (0, 0, 255)
                cv2.putText(frame, "STABILITY", (50, int(h*0.18)), 2, fs, (0,0,0), thick+2)
                cv2.putText(frame, "STABILITY", (50, int(h*0.18)), 2, fs, h_col, thick)
                cv2.putText(frame, "GREEN=GOOD | RED=FAULT", (50, h-50), 2, fs*0.6, (255,255,255), 1)
    ...
    return f"### ⚖️ Stability Report\n**Result:** {seq_status}", web_tfile.name
...
def analyze_wrist_action(video_path, ball_coords=None, start_frame=0):
    ...
    frozen_apex, s_y_lock, h_y_lock, conf = None, None, None, 0
    ...
    with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                ba, bc = np.array([lm[12].x-lm[14].x, lm[12].y-lm[14].y]), np.array([lm[16].x-lm[14].x, lm[16].y-lm[14].y])
                ...
                if frozen_apex:
                    ...
                    cv2.fillPoly(overlay, [pts], (220, 220, 220)); cv2.addWeighted(overlay, 0.25, frame, 0.75, 0, frame)
                    cv2.line(frame, frozen_apex, (0, s_y_lock), (0,0,0), 2); cv2.line(frame, frozen_apex, (0, h_y_lock+120), (0,0,0), 2)

                cv2.putText(frame, f"X-FACTOR: {xf}", (50, int(h_pix*0.1)), 2, fs*1.2, (0,0,0), thick+2)
                cv2.putText(frame, f"X-FACTOR: {xf}", (50, int(h_pix*0.1)), 2, fs*1.2, (255,0,255), thick)
                cv2.putText(frame, f"{ang}deg", (curr_re[0]+25, curr_re[1]), 2, fs, (0,0,0), thick+2)
                cv2.putText(frame, f"{ang}deg", (curr_re[0]+25, curr_re[1]), 2, fs, (0,255,255), thick)

                if not is_downswing:
                    if lm[16].y < max_h: max_h = lm[16].y
                    elif lm[16].y > (max_h + 0.05): is_downswing = True
                    wrist_trail.append(curr_rw)
                for i in range(1, len(wrist_trail)): cv2.line(frame, wrist_trail[i-1], wrist_trail[i], (0, 255, 255), 3)
