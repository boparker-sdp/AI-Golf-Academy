...
def analyze_diagnostic_swing(video_path, club_type=None):
    """Stability & Sequence scan for Face-On or DTL views."""
    cap = cv2.VideoCapture(video_path)
    w, h, fps = int(cap.get(3)), int(cap.get(4)), int(cap.get(5))
    
    # Dynamic Scaling
    fs = h / 1000  # Font Scale
    thick = int(2 * fs)
    ...
    with mp_pose.Pose(min_detection_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if res.pose_landmarks:
                ...
                cv2.putText(frame, f"SEQ: {seq_status}", (50, int(h*0.1)), 2, fs*1.2, (255, 255, 0), thick)
...
def analyze_wrist_action(video_path, ball_coords=None, start_frame=0):
    """Anatomy Plane & X-Factor Lab with Big Text."""
    cap = cv2.VideoCapture(video_path)
    h_pix, w_pix = int(cap.get(4)), int(cap.get(3))
    fps = int(cap.get(5))
    fs = h_pix / 1000  # Font Scale
    thick = int(2 * fs)
    ...
    with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                cv2.putText(frame, f"HINGE: {ang}deg", (curr_re[0]+20, curr_re[1]), 2, fs, (0,0,0), thick+2)
                cv2.putText(frame, f"HINGE: {ang}deg", (curr_re[0]+20, curr_re[1]), 2, fs, (0,255,255), thick)
